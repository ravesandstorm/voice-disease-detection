...
# pip install fastapi uvicorn librosa tensorflow matplotlib pydantic numpy scipy
# install ffmpeg
...
async def predict_audio(file: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            # First attempt: try using scipy.io.wavfile
            from scipy.io import wavfile
            sample_rate, audio_data = wavfile.read(audio_io)
            
        except Exception as e:
            # If that fails, try this fallback approach
            audio_io.seek(0)
            raw_bytes = audio_io.read()
            
            # Ensure the buffer length is a multiple of 2 (for int16)
            if len(raw_bytes) % 2 != 0:
                raw_bytes = raw_bytes[:-1]  # Remove last byte to make it even
                
            # Safety check for empty data
            if len(raw_bytes) == 0:
                raise ValueError("Audio data is empty")
                
            # Now the buffer size is guaranteed to be a multiple of element size
